...
class ImcController extends Zend_Controller_Action
{
	...
	public function getForm($id = false) {
		if (null === $this->_form) {
			$this->_form = new Form_Imc( array ('action' => 'add', 'method' => 'post', 'id' => 'fImc' ) );
		}
		return $this->_form;
	}
	...
	public function indexAction() {
		...
		if ($imc) {
			$paginator = Zend_Paginator::factory ( $imc );
	...
	public function editAction() {
        $this->view->title = "Editar IMC";
     	$this->view->headTitle ( $this->view->title, 'PREPEND' );
        $form = $this->getForm ();
        $form->addElement('hidden', 'cod_imc');
        ...
        if ($this->getRequest()->isPost()){
        	...
        	$cod_imc = $this->_getParam('cod_imc',0);
        	$mImc = new Model_Imc ();
        	$imc = $mImc->getImc($cod_imc);
        	if ($imc) {
				$form->populate($imc);        		
